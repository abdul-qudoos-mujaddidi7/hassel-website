    ...
    actions: {
        ...
        async updatePublication(id, formData) {
            try {
                ...
                const errorMessage =
                    err.response?.data?.message ||
                    "Failed to update publication. Please try again.";
                toast.error(errorMessage, {
                    position: "top-right",
                    autoClose: 3000,
                    hideProgressBar: false,
        ...
        async bulkDeletePublications(ids) {
            try {
                const deletePromises = ids.map((id) =>
                    axios.delete(`publications/${id}`)
                );
                await Promise.all(deletePromises);

                toast.success(
                    `${ids.length} publications deleted successfully!`,
                    {
                        position: "top-right",
                        autoClose: 3000,
                        hideProgressBar: false,
                ...
                console.error(err);
                toast.error(
                    "Failed to delete selected publications. Please try again.",
                    {
                        position: "top-right",
                        autoClose: 3000,
                        hideProgressBar: false,
        ...
        async getPublicationsByFileType(
            fileType,
            ...
            try {
                ...
                const response = await axios.get(
                    `publications/by-file-type/${fileType}?${params}`
                );
                return response.data;
            } catch (err) {
                console.error(err);
        ...
        getFileTypeLabel(type) {
            const typeOption = this.fileTypeOptions.find(
                (t) => t.value === type
            );
