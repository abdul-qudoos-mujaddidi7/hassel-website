...
const handleSave = async ({ data }) => {
    ...
    try {
        ...
        } else if (TrainingProgramsRepository.isEditMode && !coverImageSrc.value && !coverImageFile.value && originalCoverImage.value) {
        ...
        } else if (!TrainingProgramsRepository.isEditMode) {
            formDataToSend.append('cover_image', '');
        }
        ...
        } else if (TrainingProgramsRepository.isEditMode && !thumbnailImageSrc.value && !thumbnailImageFile.value && originalThumbnailImage.value) {
        ...
        if (data.start_date) {
            formDataToSend.set('start_date', new Date(data.start_date).toISOString().split('T')[0]);
        }
        if (data.end_date) {
            formDataToSend.set('end_date', new Date(data.end_date).toISOString().split('T')[0]);
        }

        if (TrainingProgramsRepository.isEditMode) {
            const response = await TrainingProgramsRepository.updateTrainingProgram(formData.id, formDataToSend);
            ...
            await TrainingProgramsRepository.createTrainingProgram(formDataToSend);
        }
    } finally {
        saving.value = false;
    }
