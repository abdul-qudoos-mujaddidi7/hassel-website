        ...
        <template #base-fields="{ formData, rules }">
            ...
            <div class="w-100">
                <v-textarea
                    ...
                    :rules="[rules.required]"
                    rows="8"
                    hint="Detailed description of the market access program"
                    persistent-hint
                ></v-textarea>
            </div>
        </template>
...
const handleSave = async ({ data }) => {
    ...
    try {
        ...
        if (MarketAccessProgramsRepository.isEditMode) {
            await MarketAccessProgramsRepository.updateMarketAccessProgram(formData.id, apiData);
        } else {
            await MarketAccessProgramsRepository.createMarketAccessProgram(apiData);
        }
    } finally {
        saving.value = false;
    }
