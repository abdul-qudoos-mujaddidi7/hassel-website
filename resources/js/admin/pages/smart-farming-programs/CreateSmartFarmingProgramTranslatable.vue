        ...
        <template #base-fields="{ formData, rules }">
            ...
            <div class="w-100">
                <v-textarea
                    ...
                    class="pb-4"
                    rows="3"
                    :hint="$t('hint_smart_farming_brief')"
                    persistent-hint
                ></v-textarea>
            </div>
            ...
            <div class="w-100">
                <v-textarea
                    ...
                    :rules="[rules.required]"
                    rows="6"
                    :hint="$t('hint_smart_farming_detailed')"
                    persistent-hint
                ></v-textarea>
            </div>
            ...
            <div class="w-100">
                <v-textarea
                    ...
                    class="pb-4"
                    rows="6"
                    :hint="$t('hint_implementation_guide')"
                    persistent-hint
                ></v-textarea>
            </div>
            ...
            <div class="w-100">
                <v-textarea
                    ...
                    class="pb-4"
                    rows="4"
                    :hint="$t('hint_sustainability_benefits')"
                    persistent-hint
                ></v-textarea>
            </div>
        </template>
    ...
    try {
        ...
        Object.keys(data).forEach(key => {
            if (key !== 'cover_image' && key !== 'thumbnail_image') {
                ...
                } else if (key === 'target_crops' && Array.isArray(data[key])) {
                    data[key].forEach((crop, index) => {
                        formDataToSend.append(`${key}[${index}]`, crop);
                    });
        ...
        } else if (SmartFarmingProgramsRepository.isEditMode && !coverImageSrc.value && !coverImageFile.value && originalCoverImage.value) {
        ...
        } else if (!SmartFarmingProgramsRepository.isEditMode) {
            formDataToSend.append('cover_image', '');
        }
        ...
        } else if (SmartFarmingProgramsRepository.isEditMode && !thumbnailImageSrc.value && !thumbnailImageFile.value && originalThumbnailImage.value) {
        ...
        if (data.application_deadline) {
            formDataToSend.set('application_deadline', new Date(data.application_deadline).toISOString().split('T')[0]);
        }

        if (SmartFarmingProgramsRepository.isEditMode) {
            const response = await SmartFarmingProgramsRepository.updateSmartFarmingProgram(formData.id, formDataToSend);
            ...
            await SmartFarmingProgramsRepository.createSmartFarmingProgram(formDataToSend);
        }
    } finally {
        saving.value = false;
    }
